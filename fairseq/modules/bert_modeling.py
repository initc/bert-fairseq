...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        # self.token_type_embeddings_appended = nn.Embedding(3, config.hidden_size)
        # self.LayerNorm is not snake-cased to stick with TensorFlow model variable name and be able to load
        # any TensorFlow checkpoint file
        self.LayerNorm = BertLayerNorm(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)

    def forward(self, input_ids, token_type_ids=None, position_id=None, position=None):
...
class PreTrainedBertModel(nn.Module):
    ...
    @classmethod
    def build_model(cls, pre_dir, *inputs, **kwargs):
...
# qsj
class BertPreTrainedModel(PreTrainedBertModel):
    """BERT model ("Bidirectional Embedding Representations from a Transformer").
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, positon_ids=None, output_all_encoded_layers=True):
        if attention_mask is None:
            attention_mask = torch.ones_like(input_ids)
        if token_type_ids is None:
            token_type_ids = torch.zeros_like(input_ids)
        ...
        embedding_output = self.embeddings(input_ids, token_type_ids, position_id=positon_ids)
        encoded_layers = self.encoder(embedding_output,
                                      extended_attention_mask,
                                      output_all_encoded_layers=output_all_encoded_layers)
        if not output_all_encoded_layers:
